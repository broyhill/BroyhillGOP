...
- **FEC “6 + 6 + 6” bundle (Ed — reference only):** **18 files only** — **6 Presidential + 6 US House + 6 US Senate** Schedule A exports (individual contributions reflecting donations to **candidate committees**), each ~two-year slice through 2015–2026 (March 2026 pull). Downloaded **in sequence** for suspected FEC row limits and Trump-era volume. **`FEC_FILES` in `pipeline/fec_raw_import.py` lists these 18 only.**
- **Do not mix party-committee FEC bulk files:** Do **not** ingest or combine **RNC, NRCC, NRSC** (or similar) bulk Schedule A files with the individual→candidate-committee pipeline. Do **not** ingest any FEC export whose rows are **not** individual donor records tied to **candidate committees** as defined for this program. Party / non-candidate-committee money belongs in **separate** workflows if ever needed — not mixed into this donor spine path. (Same bulk-ingest ban as above still applies unless Ed overrides.)
