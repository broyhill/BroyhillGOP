...
**Revision 1.1:** Added (1) street number extraction and address_block_key for donor-level same-address blocks; (2) full Pass 6 spec for cross-zip multi-address (e.g. Art Pope); (3) employer normalization layer (employer_normalize, employer_normalized in norm); (4) nickname handling: static nick_group + data-driven core.nickname_inferred. Clarified transaction-level vs donor-level dedup.
...
**pipeline.employer_normalize(employer_raw TEXT):** Returns normalized employer (suffixes stripped); see §6.1. Used in norm ETL and in identity Pass 5/6.

**Norm tables (per §6):** Must include columns: employer_raw, employer_normalized; street_number; address_block_key (donor-level same-address block = f(street_number, norm_last, zip5)).
...
### 2.9 Core Tables Used by Identity (Reference)

- **core.nick_group** — Static nickname → canonical (e.g. BOB → ROBERT). Seeded from name_nickname_map.
- **core.nickname_inferred** — Data-driven: (name_a, name_b, source_system, link_method, occurrence_count). Populated when Pass 1/2 links two rows with different first names. Used in Pass 4 alongside nick_group.
...
### 5.4 Transaction-Level vs Donor-Level (Critical Distinction)

- **Transaction-level dedup (raw/staging):** Prevents the same *transaction* from being loaded twice (e.g. same report line, or same donor+committee+date+amount). This is what the dedup_key in §5.1–5.3 does. It does **not** identify "same donor across addresses."
- **Donor-level identity (norm/core):** Identifies the same *person* across multiple transactions and addresses. This requires:
  - **Address block key:** For same-address matching, use **street_number + norm_last + zip5** (Eddie's rule). Raw address must be parsed to extract street_number (and optionally street_name_normalized) in norm ETL; then compute `address_block_key = f(street_number, norm_last, zip5)` (e.g. hash or concatenation). Identity passes that use "name + zip" should **optionally** use "name + address_block_key" when address is present, so same-address blocks are donor-level.
- Norm tables must carry both: transaction-level keys for raw dedup lineage, and donor-level fields (address_block_key, employer_normalized) for identity passes.
...
- **employer_raw**, **employer_normalized** (and optionally **employer_first_word**) — see §6.1
- **occupation** (from BOE/FEC raw)
- **phone** (where available)
- **street_number** (parsed from address in raw)
- **address_block_key** (f(street_number, norm_last, zip5) for donor-level same-address blocks)
...
- Apply **pipeline.employer_normalize()** and **address parsing** (street number extraction).
...
### 6.1 Employer Normalization Layer

Employer matching (Pass 5 and Pass 6) must use a **normalized** employer so "Pope Holdings Inc," "Pope Holdings LLC," and "Pope Holdings" match.

- **Function:** `pipeline.employer_normalize(employer_raw TEXT) RETURNS TEXT` (and optionally returns a second value for employer_first_word).
  - Strip legal suffixes (case-insensitive): Inc, LLC, Co, Corp, Company, L.L.C., Ltd, Limited, etc.
  - Collapse whitespace; trim.
  - Optionally: return first word (e.g. "Pope") for looser matching in employer_first_word.
- **Norm columns:** Every norm table that has employer carries: employer_raw (original), employer_normalized (result of function). Optionally employer_first_word for blocking in Pass 6.
- **Pass 5 and Pass 6:** Use employer_normalized (and employer_first_word if defined) in blocking and link conditions; never raw employer for matching.
...
- **Pass 3:** norm_last + norm_first + zip5 (or norm_last + address_block_key when address present — same-address donor block).
- **Pass 4:** norm_last + nickname_canonical + zip5 (using core.nick_group **and** core.nickname_inferred — see §7.3).
- **Pass 5:** norm_last + first_initial + zip5 + employer_normalized.
- **Pass 6:** Cross-zip multi-address — same person across different zips/addresses (e.g. Art Pope with 8–9 addresses). Full spec in §7.2.
...
- Each batch runs in a stored procedure that: finds matches, updates core.identity_clusters, sets norm.person_id, or creates new spine row. Multiple addresses per person are written to core.person_addresses.

### 7.2 Pass 6: Cross-Zip Multi-Address (Full Specification)

**Goal:** Link the same person across different zips and addresses (multi-address donors). Input: norm rows still unmatched after Passes 1–5.

**Blocking key:** (norm_last, employer_normalized) — or (norm_last) with employer used only in the link condition to avoid oversized blocks. Process in batches by this key.

**Link condition:** Two norm rows are the same person if and only if:
- They match on norm_last (and optionally employer_normalized within the block), **and**
- They share **at least one** of: voter_ncid, email (lowered), phone (normalized), or employer_normalized (if not already the blocker).

**Algorithm:**
1. For each block (norm_last, employer_normalized), gather all unmatched norm rows in that block (across all zips/addresses).
2. Build a graph: nodes = norm rows; edge between two rows if they share ncid OR email OR phone OR employer_normalized.
3. Connected components in this graph = one person each. Assign one person_id per component; create or update core.person_spine, core.identity_clusters, norm.person_id.
4. Write all addresses for that person to core.person_addresses (multiple rows per person_id).

**Edge case:** If the only shared signal is employer_normalized (no ncid/email/phone), optionally require that the same (norm_last, employer_normalized) appear in **2+ distinct zips** before linking, to reduce false positives. Configurable.

**Output:** core.person_spine and core.identity_clusters updated; norm.person_id set; core.person_addresses populated with multi-address history.

### 7.3 Nickname Handling: Static + Data-Driven

- **Static (core.nick_group):** Lookup table mapping nickname → canonical (e.g. BOB → ROBERT, BILL → WILLIAM). Used for Pass 4 and preferred name inference. Seeded from existing name_nickname_map; human-maintained.
- **Inferred (core.nickname_inferred):** When two norm rows are linked by Pass 1 or 2 (same ncid or same email) but have different first names (e.g. Robert vs Bob), record (first_name_a, first_name_b, source_system, link_method, occurrence_count) in this table. Append-only.
- **Pass 4 and preferred name:** Resolve nickname_canonical using **both** core.nick_group and core.nickname_inferred. So "Bob" and "Robert" are treated as same canonical name if either the static table says so or we have linked them by ncid/email. High-confidence inferred pairs can be promoted into nick_group by human decision.
...
- **core.person_addresses:** From norm/raw address history; **multiple rows per person_id** for multi-address donors (Pass 6).
- **core.preferred_name_cache:** From inference (source-weighted) and core.vip_overrides; nickname resolution uses core.nick_group + core.nickname_inferred.
- **core.nickname_inferred:** Populated when identity links (by ncid/email) two rows with different first names; used in Pass 4 and preferred name.
